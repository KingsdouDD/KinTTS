...
function setVolumeSettings(gain, limit, attack, release) {
    try {
        const cfg = JSON.parse(readFileSync(CONFIG_PATH, "utf-8"));
...
async function ensureModelLoaded() {
    ...
    if (!loadRes.ok)
        throw new Error(`模型加载失败：${loadRes.status}`);
}
// ─── plugin ──────────────────────────────────────────────────────────────────
    ...
    tools: (tool) => [
        ...
        tool({
            ...
            parameters: Type.Object({
                ...
                voice: Type.Optional(Type.String({ description: "音色 ID，不传则使用当前默认音色" })),
                language: Type.Optional(Type.String({ description: "语言 Chinese 或 English（默认 Chinese）" })),
            }),
            execute: async ({ text, voice, language }) => {
                ...
                if (!json.success)
                    throw new Error(`TTS 合成失败：${json.error}`);
        ...
        tool({
            ...
            execute: async () => {
                ...
                if (!res.ok)
                    throw new Error(`获取音色列表失败：${res.status}`);
                const json = await res.json();
        ...
        tool({
            ...
            execute: async ({ gain, limit, attack, release }) => {
                ...
                setVolumeSettings(gain ?? cur.gain, limit ?? cur.limit, attack ?? cur.attack, release ?? cur.release);
        ...
        tool({
            ...
            label: "qwe3 TTS 添加音色",
            description: "添加新的声音到 qwe3 TTS。音色 ID 不能与现有音色重复。" +
            ...
            parameters: Type.Object({
                ...
                reference_text: Type.Optional(Type.String({ description: "参考音频对应的文本（不填则 reference_text 为空）" })),
                language: Type.Optional(Type.String({ description: "语言 Chinese 或 English（默认 Chinese）" })),
            }),
            execute: async ({ voice_id, reference_audio, reference_text, language }) => {
                writeFileSync("/tmp/add_voice_debug.txt", JSON.stringify({ voice_id, reference_audio, reference_text, language }, null, 2));
                if (!existsSync(reference_audio))
                    throw new Error(`文件不存在：${reference_audio}`);
                await ensureService();
                const args = [voice_id, reference_audio];
                if (reference_text)
                    args.push("--text", reference_text);
                if (language)
                    args.push(language);
                writeFileSync("/tmp/add_voice_args.txt", JSON.stringify(args));
